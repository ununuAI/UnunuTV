...
async function setup(runtime, provider = "fake", { ownerAccepted = true } = {}) {
  ...
  const compilation = await runtime.app.compileAssetAuthority({
    ...
    generationParameters: { provider, model: "fake-image-v1", aspectRatio: "16:9", resolution: "1536x1024", count: 1, referenceMediaIds: [] },
    referenceBindings: []
  });
  ...
  const runtime = createLocalRuntime({
    ...
    provider: {
      async run({ request }) {
        ...
        return { status: "succeeded", artifacts: [{ kind: "image", mimeType: "image/png", bytes: Buffer.from("authority-image"), title: "角色甲.png" }] };
      },
...
test("asset authority run blocks stale compilation before Provider dispatch and releases a known Provider failure", async (context) => {
  ...
  await runtime.app.compileAssetAuthority({
    ...
    productionId: state.production.productionId,
    authorityId: state.authority.authorityId,
    generationParameters: { provider: "fake", model: "fake-image-v1", aspectRatio: "16:9", resolution: "1536x1024", count: 1, referenceMediaIds: [] },
    referenceBindings: []
  });
...
test("asset authority paid generation cannot start before current story and shot-script Owner acceptance", async (context) => {
