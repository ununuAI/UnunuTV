...
export function MomoAssetNode({ actions, boardHistory = [], displayedMediaId, mediaUrl, node, onPreviewMedia, readOnly }) {
  const [description, setDescription] = useState(node.payload?.assetDescription || "");
  const [typeQuery, setTypeQuery] = useState("");
  const typeMenuRef = useRef(null);
  const assetType = node.payload?.assetType || "character";
  ...
  useEffect(() => setDescription(node.payload?.assetDescription || ""), [node.id, node.payload?.assetDescription]);

  const saveDescription = () => {
    const next = description.trim();
    if (next !== (node.payload?.assetDescription || "")) void actions.updatePayload(node, { assetDescription: next });
  };
  ...
  return <div className="momo-asset-node">
    ...
    <section className="momo-asset-visual nodrag nopan nowheel" onWheelCapture={(event) => event.stopPropagation()}>
      {mediaUrl ? <>{/\.(mp4|mov|webm)(\?|$)/i.test(mediaUrl) ? <video muted playsInline preload="metadata" src={mediaUrl} /> : <img alt={node.title || typeLabel} src={mediaUrl} />}{!generating ? <button aria-label="替换资产媒体" disabled={readOnly} onClick={() => actions.openImport(node.id)} title="替换资产媒体" type="button"><Upload size={14} />替换</button> : null}</> : !generating ? <button className="momo-asset-media-empty" disabled={readOnly} onClick={() => actions.openImport(node.id)} type="button"><ImageIcon size={40} strokeWidth={1.25} /><span>添加批准资产图</span></button> : null}
      ...
      {generating ? <div aria-label="资产图片处理中" className="momo-asset-generation"><LoaderCircle aria-hidden="true" size={30} /><strong>图片生成中</strong><span>{node.payload?.generationMessage || "正在生成资产候选图…"}</span></div> : null}
    </section>
    </div>
  </div>;
}
