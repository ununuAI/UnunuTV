...
export default function App({ initialProjectId = null }) {
  ...
  return <div className={`video-p0-shell ${light ? "theme-light" : "theme-dark"}${projectControl.readOnly ? " project-readonly" : ""}${timelineOpen ? " timeline-open" : ""}`} style={{ "--timeline-dock-height": `${timelineHeight}px` }}>
    ...
    {timelineOpen ? <CanvasTimelineDock initialHeight={timelineHeight} notify={notify} onClose={() => setTimelineOpen(false)} onHeightChange={setTimelineHeight} onPlaybackChange={(playing) => { if (playing) void playerRef.current?.play()?.catch?.(() => {}); else playerRef.current?.pause(); }} onPreviewMedia={(preview) => { setPlayerPreview(preview); setPlayerOpen(true); }} onSeek={(milliseconds) => playerRef.current?.seek(milliseconds / 1000)} projectId={project.id} readOnly={projectControl.readOnly} selected={selected} /> : null}
