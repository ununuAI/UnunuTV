...
function AnchorStage({ actions, executionNodes, projectId, readOnly = false, shots, storyboards, units }: Pick<WorkspaceProps, "actions" | "executionNodes" | "projectId" | "readOnly" | "shots" | "storyboards" | "units">) {
  ...
  return <div className="cp-storyboard-stage">
    ...
    {batchJobs.length ? <section className="cp-storyboard-batches"><header><strong>批量生产任务</strong><small>图片可多轮探索；视频在预演、编译与预检通过后按当前精确意图提交，不会盲目重发。</small></header>{batchJobs.map((job) => {
...
function GenerateStage({ actions, units }: Pick<WorkspaceProps, "actions" | "units">) {
  const [unitId, setUnitId] = useState(units[0]?.generationUnit.generationUnitId ?? "");
  return <section className="cp-generate-gate"><Film size={34} /><h3>正式生成</h3><p>接受低模预演、提示词检查与能力预检通过后，系统为当前精确版本记录一次正式生成意图并提交。视频不是禁用项，但结果未知时不会重发，候选也不会自动通过。</p><select value={unitId} onChange={(event) => setUnitId(event.target.value)}>{units.map((record) => <option key={record.generationUnit.generationUnitId} value={record.generationUnit.generationUnitId}>{record.generationUnit.generationUnitId}</option>)}</select><button disabled={!unitId} onClick={() => void actions.runGenerationUnit(unitId)} type="button"><Play size={15} />记录意图并生成一次</button></section>;
}
