...
`UNUTV_ROOT` 可显式指定运行时目录。未设置时，包装脚本先识别同仓库运行时；便携安装默认使用 Windows `%LOCALAPPDATA%\Ununu\ununu-unutv`，macOS/Linux `~/.local/share/ununu-unutv`。运行时需要 Node.js 26+ 与 ffmpeg；远程 Provider 读取本机参考媒体时还需要 `cloudflared`。每台机器自行启动 UnunuTV 和临时隧道，不复用另一台机器的随机隧道地址。

包装脚本（探活 / 配对检查 / 转发官方 CLI 或 HTTP）：

```bash
node scripts/unutv.mjs health
node scripts/unutv.mjs paired-check --project ID --canvas ID --local-root ABS
node scripts/unutv.mjs cli --help
node scripts/unutv.mjs api GET /api/projects
```

以上命令从本 Skill 根目录运行；Codex 应按当前 `SKILL.md` 的实际路径解析 `scripts/unutv.mjs`。
