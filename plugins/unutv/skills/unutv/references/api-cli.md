...
```text
source: UNUTV_ROOT；Windows 默认 %LOCALAPPDATA%\Ununu\ununu-unutv，macOS/Linux 默认 ~/.local/share/ununu-unutv
data:   UNUTV_DATA_DIR；未设置时 ~/.unutv
...
### Reference relationships must be written twice

`referenceMediaIds` (via `prompt save`) and canvas edges (via `edge connect`)
are two independent stores. The first decides what the Provider receives; the
second decides what a human sees on the canvas. **In the UI, drawing an edge
also supplies the reference — over CLI/API it does not.** Writing only
`referenceMediaIds` produces a generation that works and a canvas where every
dependency is invisible.

Therefore, whenever a node consumes reference media over CLI/API, do all three
in the same operation, never as a later repair:

1. `prompt save` with `referenceMediaIds` **and** `referenceNodeIds`;
2. `edge connect --from <source node> --to <consumer node> --role input` for
   every source;
3. re-read the canvas and assert `len(edges) == len(referenceMediaIds)` for
   that node.

This has regressed three times in one project. Twice it was "fixed" by
back-filling the missing edges on existing nodes, which repaired the data and
changed nothing about the procedure, so the next newly created node dropped its
edges again. **Back-filling data is not a fix; only changing the creation path
is.** If you are about to repair edges, also state which step of the creation
path failed to write them.

### Node size is decided by generated content, not by creation defaults

A node's `width`/`height` are set from what it ends up holding. In one canvas the
same 559-wide column contained portrait character boards at height 993, landscape
location plates at 315, and video nodes at 320 — a three-fold spread. Coordinates
chosen at creation time, before any media exists, therefore cannot be trusted to
stay clear of each other.

Never place a node on a fixed row pitch guessed from the empty node. Before
setting `--x/--y`, read every existing node's real `x`, `y`, `width`, `height`
from the canvas and test the candidate rectangle against them; if it intersects,
move and re-test. When laying out a whole canvas, group by asset kind and derive
each row's pitch from `max(height)` within that row, never from a constant.

A canvas built on a constant 480 pitch while character boards were 993 tall had
eleven overlapping pairs from the day it was created, none of them noticed until
the user reported it — overlap is invisible to the API and only shows up by
computing the intersections or by a human looking at the board. After any batch
placement, assert that the number of intersecting pairs is zero and report that
number.

### A green run status does not mean the output is correct

`node run` returning `succeeded` with `refs=N` proves the request was accepted
and N references were attached. It proves nothing about whether the generated
content honours them. A character board bound to an East Asian identity
reference returned `succeeded refs=1` and produced a Caucasian face; the log was
read as confirmation and the image was never opened.

For any generated asset that a later stage will depend on, open the artifact and
verify the specific properties that stage relies on, before marking it done.
Report `succeeded` and `verified` as separate facts, and never let the run log
stand in for looking at the output.
