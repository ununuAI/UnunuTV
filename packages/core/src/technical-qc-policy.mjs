...
export function buildTechnicalQcReport({ graph, mediaId, probe, projectId, renderJobId }) {
  ...
  const strictVerticalDelivery = graph.preset === "h264_vertical";
  const actualChannels = Number(audio?.channels || 0);
  const actualChannelLayout = String(audio?.channel_layout || "").toLowerCase();
  const checks = graph.audioOnly ? [
    ...
    check("video_codec", "视频编码", video?.codec_name === "h264" ? "pass" : strictVerticalDelivery ? "fail" : "warning", strictVerticalDelivery ? "h264" : "h264 recommended", video?.codec_name || "missing"),
    check("audio_stream", "音频流", audio ? "pass" : strictVerticalDelivery ? "fail" : "warning", strictVerticalDelivery ? "required" : "present when mixed", audio?.codec_name || "missing"),
    check("audio_codec", "音频编码", audio?.codec_name === "aac" ? "pass" : strictVerticalDelivery ? "fail" : "warning", strictVerticalDelivery ? "aac" : "aac recommended", audio?.codec_name || "missing"),
    check(
      "audio_channels",
      "音频声道",
      actualChannels === 2 && actualChannelLayout === "stereo" ? "pass" : strictVerticalDelivery ? "fail" : "warning",
      strictVerticalDelivery ? "2ch stereo" : "stereo recommended",
      audio ? `${actualChannels || "unknown"}ch ${actualChannelLayout || "unknown"}` : "missing"
    )
  ];
